...
Ext.define('SistemaDemo.controller.Main', {
	...
	requires : ['Ext.window.MessageBox', 'Ext.tip.*'],
	stores : ['Menus'//,
	//'Companies',
	//'States'
	],
	views : ['Viewport', 'layout.Header'],
	refs : [{
		ref : 'mainPanel',
		selector : '#mainPanel'
	}, {
		...
		this.control({
			'viewport menuList' : {
				...
				afterlayout : function() {
					var me = this, className, menuList, name, record;
	...
	setActiveExample : function(className, title) {
		var me = this;
		var mainPanel = this.getMainPanel(), path, mypanel, className;
	...
	filePathFromRecord : function(record) {
		var parentNode = record.parentNode, path = record.get('text');
	...
	formatPath : function(string) {
		var result = string.split(' ')[0].charAt(0).toLowerCase() + string.split(' ')[0].substr(1), paths = string.split(' '), ln = paths.length, i;
